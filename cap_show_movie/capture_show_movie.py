...
def main():
    ...
    capture = cv2.VideoCapture('../movie/box.mov') # generate capture object
    #capture = cv2.VideoCapture(0) # generate capture object

    # 動画のサイズ，フレームレートの取得
    mov_w  = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
    mov_h  = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
    mov_fr = capture.get(cv2.CAP_PROP_FPS)
